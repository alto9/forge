...
usage() {
  cat <<'EOF' >&2
Usage: assign-issue-to-me.sh <owner/repo> <issue-number>
       assign-issue-to-me.sh <issue-number>
...
if [[ $# -eq 1 ]]; then
  ...
  if ! owner_repo="$(gh repo view --json nameWithOwner -q '.nameWithOwner' 2>/dev/null)"; then
    echo "Error: unable to resolve owner/repo. Run from a clone or pass: assign-issue-to-me.sh <owner/repo> <issue-number>" >&2
    exit 1
  fi
elif [[ $# -eq 2 ]]; then
  owner_repo="$1"
...
gh issue edit "$issue_number" --add-assignee "@me" --repo "$owner_repo"
