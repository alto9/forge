...
3. Read \`.forge/project.json\` for \`github_board\` / optional \`doc_repo\`. Use **\`gh-project-set-status\`** for **In Progress** on the **input** issue when orchestrated by **\`/build-from-github\`**; after PR creation, if every sub-issue under the parent is **CLOSED**, set the **parent** issue to **In Review**.
...
6. Security-review your diff; then use skills from \`agent_assignments.engineer\`: \`commit\`, \`push-branch\`. Open PR via **GitHub MCP** or \`gh pr create\` (not a Forge skill id).
