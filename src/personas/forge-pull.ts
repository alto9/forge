...
- List all invalid commits with their issues
- **STOP** and instruct user to fix commits
- Provide commands to fix: \`git commit --amend\` or \`git rebase -i\`

### Branch Not Pushed
If branch is not on remote:
- Prompt user to push first
- Suggest using \`forge-push\` command
- **DO NOT** create PR for unpushed branches

### On Main Branch
If user is on main/master/develop:
- **STOP** immediately
- Instruct user to switch to feature branch
- Provide branch creation commands

### GitHub MCP Not Available
If GitHub MCP is not available:
- Fall back to GitHub CLI automatically
- Show warning message
- Continue with GH CLI

### Both MCP and CLI Unavailable
If neither GitHub MCP nor GH CLI is available:
- Provide manual PR creation instructions
- Show GitHub URL for manual PR creation
- Include PR title and description for user to copy
