...
Update the UI permission logic to allow specs, diagrams, models, actors, and contexts to be created and edited at any time, regardless of whether an active session exists. These file types are informative (not directive) and should always be editable.

## Context

Previously, Forge Studio required an active session to edit specs, diagrams, and models. The new design clarifies that:
- **Features are directive**: Require active session (they represent code changes)
- **Specs/Diagrams/Models/Actors/Contexts are informative**: Always editable (they provide guidance)
...
2. Update the logic to return `true` for specs, diagrams, models, actors, and contexts regardless of session status
...
5. Remove read-only mode indicators for specs/diagrams/models/actors/contexts
...
- `packages/vscode-extension/src/webview/studio/pages/ModelsPage.tsx` - Remove session requirement
...
- [ ] Models can be created without active session
- [ ] Models can be edited without active session
