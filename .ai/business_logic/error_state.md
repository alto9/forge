...
- Run input invalid: the selected workflow definition declares required `run_inputs[]`, but the user has not supplied valid values before start. This state is reported before Temporal run creation and uses the same diagnostic shape as definition validation.
...
- Workflow start blocked: Forge cannot pass a pre-Temporal gate for the selected workflow. Definition invalid, run input invalid, configuration invalid, Temporal readiness failure, worker readiness failure, and duplicate in-flight start suppression all block run creation and reuse the existing diagnostic shape for their source.
- Workflow start failed: Temporal start was attempted after readiness passed, but failed before Temporal returned run identity. Forge reports the redacted failure through the existing Temporal diagnostic surfaces and does not create local run-index state.
